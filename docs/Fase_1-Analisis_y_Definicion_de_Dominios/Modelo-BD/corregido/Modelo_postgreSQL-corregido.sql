...
-- [FIX-01] Added is_primary flag to identify the active travel document per person/type.
-- A partial unique index enforces that only one primary document exists per person per document type.
CREATE TABLE person_document (
    ...
    issued_on date,
    expires_on date,
    is_primary boolean NOT NULL DEFAULT false,  -- [FIX-01]
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
...
-- [FIX-01] Partial unique index: only one primary document per person per type at a time.
CREATE UNIQUE INDEX uq_person_document_primary
    ON person_document(person_id, document_type_id)
...
-- [FIX-02] Added is_current boolean with partial unique index to allow O(1) lookup
-- of the active tier without requiring MAX(assigned_at) aggregation.
-- The original unique key on (loyalty_account_id, assigned_at) is fragile under
-- concurrent writes at the same millisecond. The partial index guarantees at most
-- one active tier per account at any moment.
CREATE TABLE loyalty_account_tier (
    ...
    is_current boolean NOT NULL DEFAULT false,  -- [FIX-02]
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_loyalty_account_tier_assignment UNIQUE (loyalty_account_id, assigned_at),
    CONSTRAINT ck_loyalty_account_tier_dates CHECK (expires_at IS NULL OR expires_at > assigned_at)
);

-- [FIX-02] At most one current tier per loyalty account.
CREATE UNIQUE INDEX uq_loyalty_account_tier_current
    ON loyalty_account_tier(loyalty_account_id)
    WHERE is_current = true;

-- [FIX-03] Added balance_after column to snapshot the running mile balance after each
-- transaction. Avoids full table scans (SUM over entire history) when retrieving
-- current balance. The application must maintain this value on INSERT.
CREATE TABLE miles_transaction (
    ...
    balance_after integer NOT NULL,  -- [FIX-03] running balance snapshot
...
-- [FIX-04] Added departure_gate_id and arrival_gate_id to link operational gate
-- assignments directly to a flight segment. Without this, the schema has no way to
-- associate a boarding gate with the flight that uses it, making check-in and
-- boarding_validation records impossible to trace back to the correct gate operationally.
-- Both columns are nullable because gate assignments are made later in the ops workflow.
CREATE TABLE flight_segment (
    ...
    departure_gate_id uuid REFERENCES boarding_gate(boarding_gate_id),  -- [FIX-04]
    arrival_gate_id uuid REFERENCES boarding_gate(boarding_gate_id),  -- [FIX-04]
...
CREATE TABLE fare (
    ...
    -- NOTE: Overlapping validity periods for the same (airline, origin, destination, fare_class)
    -- cannot be prevented with a simple CHECK constraint. Enforce via application logic or
    -- a btree_gist EXCLUDE constraint: EXCLUDE USING gist (..., daterange(valid_from, valid_to) WITH &&)
);
...
-- [FIX-05] New bridge table: invoice_payment
-- The original schema had no way to link a payment to a specific invoice.
-- Both payment and invoice are children of sale, but reconciliation (which payment
-- covers which invoice, and for how much) requires an explicit relationship.
-- allocated_amount supports partial payments and split billing scenarios.
CREATE TABLE invoice_payment (
    ...
    CONSTRAINT ck_invoice_payment_amount CHECK (allocated_amount > 0)
);
...
COMMENT ON TABLE loyalty_account_tier IS 'Historial de asignacion de nivel. El campo is_current y su indice parcial permiten lookup O(1) del tier activo sin agregar sobre assigned_at.';
COMMENT ON TABLE invoice_line         IS 'Detalle facturable sin totales derivados persistidos, para preservar 3FN.';
COMMENT ON TABLE invoice_payment      IS 'Tabla puente para reconciliacion de pagos contra facturas. Soporta pagos parciales y escenarios de facturacion dividida.';
COMMENT ON TABLE miles_transaction    IS 'balance_after persiste el saldo acumulado post-transaccion para evitar SUM() completo sobre el historial al consultar saldo actual.';
COMMENT ON TABLE person_document      IS 'is_primary identifica el documento de viaje activo por persona y tipo. Indice parcial garantiza unicidad de primario.';
COMMENT ON TABLE flight_segment       IS 'departure_gate_id y arrival_gate_id vinculan operativamente la pista de abordaje a cada segmento de vuelo.';
COMMENT ON TABLE loyalty_account_tier IS 'is_current con indice parcial reemplaza la necesidad de MAX(assigned_at) para obtener el tier vigente.';

-- ============================================
-- INDEXES  (originals + new)
-- ============================================
...
CREATE INDEX idx_customer_airline_id            ON customer(airline_id);                          -- [FIX-06] new
CREATE INDEX idx_loyalty_program_airline_id     ON loyalty_program(airline_id);
CREATE INDEX idx_loyalty_tier_program_id        ON loyalty_tier(loyalty_program_id);             -- [FIX-06] new
...
CREATE INDEX idx_miles_transaction_occurred_at  ON miles_transaction(occurred_at);               -- [FIX-06] new
...
-- Flights
CREATE INDEX idx_flight_airline_id                   ON flight(airline_id);                      -- [FIX-06] new
...
CREATE INDEX idx_flight_segment_departure_gate_id    ON flight_segment(departure_gate_id);       -- [FIX-04] new
CREATE INDEX idx_flight_segment_arrival_gate_id      ON flight_segment(arrival_gate_id);         -- [FIX-04] new
...
CREATE INDEX idx_fare_route                       ON fare(origin_airport_id, destination_airport_id); -- [FIX-06] new
CREATE INDEX idx_fare_validity                    ON fare(valid_from, valid_to);                  -- [FIX-06] new
...
CREATE INDEX idx_ticket_fare_id                   ON ticket(fare_id);                             -- [FIX-06] new
...
CREATE INDEX idx_invoice_payment_invoice_id   ON invoice_payment(invoice_id);                    -- [FIX-05] new
CREATE INDEX idx_invoice_payment_payment_id   ON invoice_payment(payment_id);                    -- [FIX-05] new